...
if you choose the command line route then nixOS provides a handy <Code code="nixos-generate-config" inline /> that generates a base configuration for you to edit, it makes things so much easier, it has things like <Code code="sound.enable = true" inline /> already in there for you toy around with. almost everything you do to install nixOS (except partitioning the drives) is done in the 2 generated <Code code=".nix" inline /> files
...
nixOS has many options for doing things other than installing packages, for example, I was able to get my ThinkPad's TrackPoint working without too much hassle by adding 3 config options in the <Code code="configuration.nix" inline /> file and rebuilding my config using <Code code="nixos-rebuild" inline />:
...
Most of the documentation references <Code code="nix-env" inline />, a tool that all nixOS users recommend against using due to its slowness and the problems that it causes, yet the devs haven't removed it from the documentation.
