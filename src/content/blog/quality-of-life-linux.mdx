...
Both problems can be solved with <Code lang="bash" code="sudoedit" inline />, simply run <Code lang="bash" code="sudoedit /etc/your-protected-file.sh" inline /> and it will open the file in your preferred `$EDITOR` with your config.

In case, it opens in `nano` then you likely didn't set up your <Code code="$EDITOR" lang="bash" inline /> correctly, add the following line to your shell config and `source` it to fix the issue.
