...
- **mpc**: it's a commandline tool, so you can use commands like <Code code="mpc next" inline /> and <Code code="mpc update" inline /> to use it, it's really useful, for configuring system keybindings to pause/go to the next or previous song
- **ncmpcpp**: it's similar to mpc as it runs in the terminal too, but the difference is that it has an actual TUI interface (like <Code code="htop" inline /> for example)
