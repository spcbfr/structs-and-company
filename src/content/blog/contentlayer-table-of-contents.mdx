...
First go to your <Code code="let x = 2" lang="js"  inline />, specifically the <Code code="makeSource" inline /> function, it should look something like this
...
<Callout title="Explaining the Regex 🗿" client:load>
  In short, <Code code="Hi ## It's me" inline /> and <Code code="###nospace" inline /> won't match, but <Code code="# Hello World" inline /> will. Along with it 2 properties will be returned. <Code code='flag = "#"' inline /> and `content= "Hello World"{:js}` which we'll be using later.
